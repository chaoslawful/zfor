...
loop(Parent, DbgOpts, OldState) ->
	DbgOpts2 = sys:handle_debug(
		...
		{"prepare to reload configuration", OldState}
	),
	% 重新载入配置数据
	{Reloaded, State} = zfor_config:reload_conf(OldState),
	DbgOpts3 = sys:handle_debug(
		DbgOpts2, {?MODULE, debug_output}, ?MODULE,
		if	Reloaded -> {"configuration is refreshed", State};
	...
	DbgOpts4 = sys:handle_debug(
		DbgOpts3, {?MODULE, debug_output}, ?MODULE,
		{
	...
	DbgOpts5 = sys:handle_debug(
		DbgOpts4, {?MODULE, debug_output}, ?MODULE,
		{
			"prepare to sleep",
			timeout, Timeout,
			State2
	...
	NewProcDict = dict:fold(
		fun
			(Key, #proc_info{ts = TS1,pid = Pid} = Val, OldDict) when TS1 =:= TS ->
				IsAlive = if	Pid =:= undefined -> false;
			...
			({Type, Data}, #proc_info{pid = Pid}, OldDict) when Pid =/= undefined ->
				% 当前项检查时间没有更新，强制杀死对应进程，并将其从新字典中删除
				exit(Pid, kill),
				if	Type =:= ?PROCDICT_TYPE_VHOST_CHECKER ->
...
stop_all_proc(State) ->
	...
	dict:fold(
		fun
			(_, #proc_info{pid = Pid}, Dummy) when Pid =/= undefined ->
				exit(Pid,kill),
				Dummy;
			(_, _, Dummy) -> Dummy
		end,
